...
from src.gold.create_metrics import create_may_hourly_passenger_metric, create_monthly_amount_metric, write_gold_table

spark.sql(f"CREATE SCHEMA IF NOT EXISTS {CATALOG}.{GOLD_SCHEMA}")

silver_df = spark.table(SILVER_TABLE)
monthly_amount_df = create_monthly_amount_metric(silver_df)
hourly_passenger_df = create_may_hourly_passenger_metric(silver_df)
